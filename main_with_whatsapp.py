...
# To use this: pip install twilio
from twilio.rest import Client

def _send_otp_whatsapp(to_phone, otp_code):
    ...
    try:
        ...
        if not account_sid or not auth_token:
            print("⚠️ Twilio Keys missing. Falling back to Mock.", flush=True)
            print(f"👉 MOCK WHATSAPP: To={to_phone} | Code={otp_code}", flush=True)
            return True
        ...
        print(f"✓ WhatsApp Sent! SID: {message.sid}", flush=True)
        return True

    except Exception as e:
        print(f"✗ WhatsApp Failed: {e}", flush=True)
        print(f"👉 MOCK WHATSAPP (Fallback): To={to_phone} | Code={otp_code}", flush=True)
        return True
...
def send_email_otp(to_email, code):
    # In this version, we treat 'to_email' as phone for demo purposes
    return _send_otp_whatsapp(to_email, code)
...
def read_root():
    try:
        ...
        if not os.path.exists(index_path):
            return {"error": f"Critical: static/index.html not found at {index_path}"}
